...
def sample_data_dir(tmp_path):
    ...
    idp_data = pd.DataFrame({
        ...
        'Technique': ['Brute Force', 'Valid Accounts'],
        'Description': [
            'IDP specific brute force description.',
            'IDP specific valid accounts description.'
        ]
    })
...
def test_load_csv_mitre(data_processor, sample_data_dir):
    ...
    assert all(col in data_processor.mitre_data.columns 
              for col in CSV_SETTINGS['mitre']['required_columns'])
...
def test_load_csv_idp(data_processor, sample_data_dir):
    ...
    assert all(col in data_processor.idp_data.columns 
              for col in CSV_SETTINGS['idp']['required_columns'])
...
def test_load_csv_audit(data_processor, sample_data_dir):
    ...
    assert all(col in data_processor.audit_data.columns 
              for col in CSV_SETTINGS['audit']['required_columns'])
...
def test_calculate_correlation_score(data_processor):
    ...
    technique = pd.Series({
        ...
        'Description': 'Test brute force attack description'
    })
    operation = pd.Series({
    ...
    # Calculate score
    score = data_processor._calculate_correlation_score(
