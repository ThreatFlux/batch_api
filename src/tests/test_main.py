...
def test_parse_args() -> None:
    ...
    with patch("sys.argv", ["threat_model"]):
        ...
        assert args.sections == ["Authentication Mechanisms", "Data Access Controls", "Application Security"]
...
def test_main_success(mock_load_dotenv: Mock, mock_generator: Mock) -> None:
    ...
    mock_env = {"ANTHROPIC_API_KEY": "test-key"}
    with patch.dict(os.environ, mock_env):
        ...
        with patch("sys.argv", ["threat_model"]):
            ...
            assert (
                mock_generator_instance.generate_threat_model.called
                or mock_generator_instance.generate_threat_model_batch.called
            )
...
def test_main_batch_mode(mock_load_dotenv: Mock, mock_generator: Mock) -> None:
    ...
    with patch.dict(os.environ, mock_env):
        ...
        test_args = ["threat_model", "--batch", "--sections", "Section1", "Section2"]
        with patch("sys.argv", test_args):
            main()
